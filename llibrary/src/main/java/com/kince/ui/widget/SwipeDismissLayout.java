...
//                    mOnEnterAnimationCompleteListener);
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mSwiping) {
                    if (getContext() instanceof Activity) {
                        ...
                        forceConvertToTranslucent(((Activity) getContext()));
                    }
    ...
    protected void cancel() {
        if (getContext() instanceof Activity) {
            //((Activity) getContext()).convertFromTranslucent();
            forceConvertActivityFromTranslucent(((Activity) getContext()));
        }
    ...
    public static void forceConvertToTranslucent(Activity activity) {
        try {
            Method method = Activity.class.getDeclaredMethod("convertToTranslucent", new Class<?>[]{});
            method.setAccessible(true);
            method.invoke(activity, new Object[]{});
        } catch (Throwable ignored) {
        }
